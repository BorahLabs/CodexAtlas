...
it('dont make a new request when the file is processed', function () {
    $user = User::factory()->inFreeTrialMode()->create();
    $project = Project::factory()->create([
        'team_id' => $user->currentTeam->id,
    ]);
    ...
    expect($branch->systemComponents()->first()->path)->toBe('composer.json');
    expect($branch->systemComponents()->first()->markdown_docs)->not->toBeEmpty();
    $systemComponentUpdatedAt = $branch->systemComponents()->first()->getOriginal('updated_at');
    ProcessSystemComponent::make()->handle($branch, $file, 0);
    $secondSystemComponentUpdatedAt = $branch->systemComponents()->first()->getOriginal('updated_at');
    expect(Carbon::parse($systemComponentUpdatedAt)->eq($secondSystemComponentUpdatedAt))->toBe(true);
});
