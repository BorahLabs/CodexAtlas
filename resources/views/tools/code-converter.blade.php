<x-web-layout>
    @section('title', 'Free ' . $tool->name() . ' | CodexAtlas')
    <section class="py-8">
        <div class="flex items-center justify-center space-x-12 my-8">
            <img src="{{ $tool->from->imageUrl() }}" alt="{{ $tool->from->name() }}"
                class="w-16 h-auto brightness-0 invert" />
            <span class="text-white text-4xl font-bold">to</span>
            <img src="{{ $tool->to->imageUrl() }}" alt="{{ $tool->to->name() }}" class="w-16 h-auto brightness-0 invert" />
        </div>
        <div class="w-full sm:max-w-2xl mt-6 p-6 mx-auto shadow-md overflow-hidden sm:rounded-lg prose prose-invert">
            <h1 class="text-4xl font-bold text-center">Free {{ $tool->name() }}</h1>
            <p class="text-2xl text-center font-medium mt-2">
                No email required. 100% free. Done in 30 seconds.
            </p>
            <p class="text-lg text-center">
                Transform your code from {{ $tool->from->name() }} to {{ $tool->to->name() }} with our free AI-based
                code convertion tool.
                If you like what you see, we also <a href="{{ route('homepage') }}">create documentation for your
                    code</a>!
                We don't ever store your code or any representation of it in our databases, but it will be shared with
                the LLM of our choice for processing.
            </p>
        </div>
